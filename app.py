...
# --- 4. CSS (Styles Ajustés et Corrigés) ---
...
/* --- CORRECTION POLICES : Cible uniquement les textes, pas les conteneurs système --- */
...
p, label, button, .stMarkdown {{ font-family: 'Rajdhani', sans-serif !important; }}
...
/* --- NAVIGATION PROPRE (Boutons Rectangulaires) --- */
...
def render_sidebar():
    with st.sidebar:
        st.markdown("<h2 style='border:none; text-align:center'>💠 PIONEER</h2>", unsafe_allow_html=True)
        if st.session_state.current_pilot:
            st.markdown(f"<div style='text-align:center; color:#00d4ff; font-weight:bold; margin-bottom:20px;'>PILOTE: {st.session_state.current_pilot}</div>", unsafe_allow_html=True)
            
            # Navigation Propre
            nav_options = ["CATALOGUE", "MON HANGAR", "FLOTTE CORPO"]
            current_index = 0
            if st.session_state.menu_nav in nav_options:
                current_index = nav_options.index(st.session_state.menu_nav)
            
            nav = st.radio("NAVIGATION", nav_options, index=current_index, label_visibility="collapsed")
            
            if nav != st.session_state.menu_nav:
                st.session_state.menu_nav = nav
...
def catalogue_page():
    ...
    with col_cart:
        ...
        if not st.session_state.cart:
            ...
            cart_counts = {}
            for item in st.session_state.cart:
                key = (item['name'], item['source'], item['insurance'], item.get('price_disp', 'N/A'))
...
def corpo_fleet_page():
    ...
    if not df_standard.empty:
        ...
        # Remplacement des tabs par selectbox pour éviter le bug visuel
        selected_role = st.selectbox("Filtrer par Rôle", ["Tout afficher"] + all_roles)

        if selected_role == "Tout afficher":
            df_to_show = df_standard
        else:
            df_to_show = df_standard[df_standard["Rôle"] == selected_role]
    ...
    # Correction de la colonne image (medium width)
    st.dataframe(
        final_view,
        column_config={
