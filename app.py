...
def normalize_db_schema(db: dict) -> dict:
    """Normalise la structure de la DB avec les nouveaux champs Crew."""
    db.setdefault("users", {})
    db.setdefault("fleet", [])
...
def update_ship_attributes(pilot, ship_name, source, old_ins, old_disp, old_need, new_ins, new_disp, new_need):
    """Met à jour l'assurance, la disponibilité et le statut NeedCrew."""
    updated = False
    for s in st.session_state.db["fleet"]:
        # On cherche le vaisseau exact (y compris avec son ancien statut NeedCrew)
        if (s["Propriétaire"] == pilot and 
            s["Vaisseau"] == ship_name and 
            s["Source"] == source and 
            s["Assurance"] == old_ins and
            s["Dispo"] == old_disp and
            s.get("NeedCrew", False) == old_need):
            
            s["Assurance"] = new_ins
            s["Dispo"] = bool(new_disp)
...
def toggle_crew_signup(ship_id, pilot_name):
    """Gère l'inscription/désinscription d'un membre d'équipage."""
    for s in st.session_state.db["fleet"]:
        if s["id"] == ship_id:
            if pilot_name in s["CrewList"]:
                s["CrewList"].remove(pilot_name)
                st.toast("Vous avez quitté le poste.", icon="👋")
            else:
                s["CrewList"].append(pilot_name)
                st.toast("Bienvenue dans l'équipage !", icon="🚀")
...
/* --- POLICES CIBLÉES (POUR NE PAS CASSER LE TABLEAU) --- */
...
/* --- LOCK SIDEBAR (CACHE LES BOUTONS) --- */
        ...
        if not st.session_state.cart:
            ...
            for (c_name, c_src, c_ins, c_prc), count in cart_counts.items():
                ...
                <div style="background:rgba(255,255,255,0.05); padding:6px; border-radius:4px; margin-bottom:4px; border-left:3px solid #00d4ff; display:flex; justify-content:space-between; align-items:center;">
                    <div><b>{c_name}</b> <span style="background:#333; padding:1px 5px; border-radius:3px;">x{count}</span><br><span style="font-size:0.8em; color:#aaa;">{c_src} | {c_ins} | {c_prc}</span></div>
...
def my_hangar_page():
        ...
        if my_fleet:
            ...
            if df.empty:
                ...
                def render_fleet_grid_editable(dataframe, is_flagship=False):
                    ...
                    # Groupement incluant NeedCrew pour différencier les états
                    grp = dataframe.groupby(['Vaisseau', 'Source', 'Assurance', 'Dispo', 'NeedCrew']).agg({'id': 'count', 'Image': 'first'}).reset_index().rename(columns={'id': 'Quantité'})
                    ...
                    for i, row in grp.iterrows():
                        with cols[i % 3]:
                            ...
                            crew_badge = "<span class='crew-tag'>SEARCH CREW</span>" if need_crew else ""
                            ...
                            # CONTROLES
                            c_edit, c_opts, c_del = st.columns([1.5, 2, 0.5])
                            ...
                            with c_opts:
                                new_disp = st.toggle("Prêt", value=dispo, key=f"d_{i}_{name}_{source}")
                                new_need = st.toggle("📢 Search Crew", value=need_crew, key=f"nc_{i}_{name}_{source}")

                            if new_ins != insurance or new_disp != dispo or new_need != need_crew:
                                update_ship_attributes(st.session_state.current_pilot, name, source, insurance, dispo, need_crew, new_ins, new_disp, new_need)

                            with c_del:
                                if st.button("🗑️", key=f"del_{i}_{name}_{source}_{dispo}", help="Retirer un exemplaire"):
                                    to_remove_id = None
                                    for s in st.session_state.db["fleet"]:
                                        if (s["Propriétaire"] == st.session_state.current_pilot and s["Vaisseau"] == name and s["Source"] == source and s["Assurance"] == insurance and s["Dispo"] == dispo and s.get("NeedCrew", False) == need_crew):
                                            to_remove_id = s["id"]
                                            break
                                    if to_remove_id:
    ...
    # --- ONGLET 2 : ACQUISITION (CALCULATEUR RESTAURÉ) ---
    with tab_acq:
        ...
        def update_balance_slider(): st.session_state.calc_balance = st.session_state.h_slider
        def update_balance_num(): st.session_state.calc_balance = st.session_state.h_num

        c1, c2 = st.columns([1, 2])
        target_cost = 0
        if current_selection != "—" and current_selection in SHIPS_DB:
            t_price = SHIPS_DB[current_selection].get('auec_price', 0)
            if isinstance(t_price, (int, float)) and t_price > 0: target_cost = t_price
        ...
        with c2:
            ...
            if target_cost > 0:
                ...
                st.progress(pct)
                if bal >= target_cost: st.markdown("""<div style="background:rgba(0,255,0,0.2); border:1px solid #0f0; color:#fff; padding:10px; border-radius:5px; text-align:center; margin-top:10px;">🚀 FONDS SUFFISANTS POUR L'ACHAT !</div>""", unsafe_allow_html=True)
                else: st.markdown(f"*Il manque {target_cost - bal:,.0f} aUEC*")
            else:
        ...
        with cols[i % 3]:
            ...
            <div class="corpo-card crew-card">
                ...
                <div class="corpo-card-header" style="background:rgba(255,0,85,0.15);">
                    ...
                    <span class="crew-tag">CAPITAINE: {owner}</span>
                </div>
                <div class="corpo-card-body">
                    <div style="margin-bottom:10px;"><strong>Équipage actuel ({len(crew_list)}):</strong><br>{members_html}</div>
                </div>
            </div>""", unsafe_allow_html=True)
            
            if owner == current_user:
                st.button("👑 C'est votre vaisseau", key=f"own_{ship['id']}", disabled=True, use_container_width=True)
            else:
                if current_user in crew_list:
                    if st.button(f"❌ QUITTER LE POSTE", key=f"leave_{ship['id']}", use_container_width=True):
                        toggle_crew_signup(ship['id'], current_user)
                else:
...
def corpo_fleet_page():
    ...
    total_auec_global = 0
    for _, row in df.iterrows():
        if row['Source'] == 'STORE': total_usd_global += get_current_ship_price(row['Vaisseau'], 'USD')
        elif row['Source'] == 'INGAME': total_auec_global += get_current_ship_price(row['Vaisseau'], 'aUEC')
    ...
    # --- 3 ONGLETS : VISUEL / TABLEAU / MEMBRES ---
    ...
    with tab_visu:
        ...
        if not df_standard.empty:
            ...
            if selected_role_view == "Tout afficher": df_to_show = df_standard
            else: df_to_show = df_standard[df_standard["Rôle"] == selected_role_view]
                
            grp_role = df_to_show.groupby(['Vaisseau']).agg({'Propriétaire': lambda x: sorted(x.unique()), 'id': 'count', 'Image': 'first'}).reset_index()
    ...
    with tab_table:
        ...
        with c_search: search = st.text_input("🔍 Filtrer la liste globale (Vaisseau, Pilote, Rôle)", "")

        df_filtered = df.copy()
        if search:
            m = search.lower()
            df_filtered = df_filtered[df_filtered["Vaisseau"].str.lower().str.contains(m) | df_filtered["Propriétaire"].str.lower().str.contains(m) | df_filtered["Rôle"].str.lower().str.contains(m)]

        grp = df_filtered.groupby(['Vaisseau', 'Source', 'Rôle']).agg({'Propriétaire': lambda x: ', '.join(sorted(x.unique())), 'id': 'count', 'Image': 'first'}).reset_index().rename(columns={'id': 'Quantité'})
    ...
    with tab_members:
        ...
        # On récupère tous les utilisateurs (ceux qui ont des vaisseaux et ceux qui ont juste un compte)
        ...
        for p in all_pilots:
            ...
            val_usd = p_ships[p_ships['Source']=='STORE']['Prix_USD'].sum()
            val_auec = p_ships[p_ships['Source']=='INGAME']['Prix_aUEC'].sum()
            target_p = st.session_state.db["user_data"].get(p, {}).get("acquisition_target", "Aucun")
            
            data_members.append({
                "Pilote": p,
                "Vaisseaux": len(p_ships),
                "Valeur Pledges": f"${val_usd:,.0f}",
                "Valeur InGame": f"{val_auec:,.0f} aUEC",
