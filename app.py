...
def my_hangar_page():
    ...
    if not my_fleet:
        ...
        if not df_store.empty:
            st.data_editor(df_store_display, column_config=editable_columns_base, disabled=disabled_cols, hide_index=True, use_container_width=True, key="store_hangar_editor")
        else:
        ...
        if not df_ingame.empty:
            st.data_editor(df_ingame_display, column_config=editable_columns_base, disabled=disabled_cols, hide_index=True, use_container_width=True, key="ingame_hangar_editor")
        else:
            st.info("Aucun vaisseau acheté en jeu dans votre hangar.")

        # --- SAUVEGARDE GLOBALE DES VAISSEAUX POSSÉDÉS ---
        st.markdown("---")
        # NOTE: La logique de fusion pour la sauvegarde est omise ici pour la clarté mais reste dans la fonction process_fleet_updates
        
        if st.button("💾 ACTUALISER LA FLOTTE (SAUVEGARDER & SUPPRIMER)", type="primary", use_container_width=True):
            # Recréer le combined_edited DataFrame manuellement ici pour la démo
            combined_edited = pd.concat([df_store, df_ingame], ignore_index=True)
...
def corpo_fleet_page():
    ...
    # 1. Regrouper les lignes pour la LISTE DÉTAILLÉE
    detail_data = display_df.groupby(['Vaisseau', 'Marque', 'Rôle', 'Source', 'Assurance']).agg(
        ...
        Visuel=('Visuel', 'first'),
        # Image ('Image', 'first') is technically the path, but 'Visuel' now holds the Base64 needed
    ).reset_index()

    # 2. Préparer le DataFrame pour l'affichage
    ...
    display_for_table['Aperçu'] = detail_data['Visuel']
